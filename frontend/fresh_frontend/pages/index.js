...
// Actual backend routes (verified):
//   GET  /api/v1/books/
//   POST /api/v1/books/
//   GET  /api/v1/members/
//   POST /api/v1/members/member_add
//   GET  /api/v1/borrow                      ← lists all borrow records
//   GET  /api/v1/members/{id}/borrowed        ← borrows for one member
//   POST /api/v1/borrow
//   POST /api/v1/return
//
// BorrowRecord fields: id, book_id, member_id, borrow_date, return_date, status
...
export default function Home() {
  ...
  const loadBooks = useCallback(async () => {
    const b = await get('/books/')
    setBooks(b)
  }, [])

  const loadMembers = useCallback(async () => {
    const m = await get('/members/')
    setMembers(m)
  }, [])

  // Load all borrows at once from /api/v1/borrow
  // then join member names client-side
  const loadBorrows = useCallback(async (membersList) => {
    const recs = await get('/borrow')
    // attach member name for display
    const withNames = recs.map(r => ({
      ...r,
      member_name: (membersList || members).find(m => m.id === r.member_id)?.name || `Member #${r.member_id}`,
    }))
    setBorrows(withNames)
  }, [members])
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const withNames = recs.map(r => ({
        ...r,
        member_name: m.find(mem => mem.id === r.member_id)?.name || `Member #${r.member_id}`,
      }))
      setBorrows(withNames)
    } catch (e) {
    ...
    try {
      ...
      if (modal === 'borrow') {
        if (!form.book_id || !form.member_id)
          throw new Error('Please select a book and a member')
        await post('/borrow', {
  ...
  // BorrowRecord uses borrow_date / return_date / status (not borrowed_at / returned_at)
  ...
  const filteredBorrows = borrows.filter(r =>
    filter === 'active'   ? isActive(r) :
    filter === 'returned' ? !isActive(r) : true
  )
          ...
          <div style={{ background: 'var(--bg-primary)', borderRadius: 'var(--radius-lg)', border: '0.5px solid var(--border-light)', padding: 24 }}>
                ...
                <div style={{ display: 'flex', gap: 3, background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', padding: 3 }}>
                  {[['all','All'],['active','Active'],['returned','Returned']].map(([v,l]) => (
                    <button key={v} onClick={() => setFilter(v)} style={{
                      ...
                      {l}
                      {v === 'active' && activeCount > 0 &&
                        <span style={{ marginLeft: 4, background: 'var(--color-red-bg)', color: 'var(--color-red)', borderRadius: 10, padding: '0 5px', fontSize: 11 }}>{activeCount}</span>}
                    </button>
                  ))}
                </div>
              )}
            ...
            {!loading && tab === 'books' && (
              filteredBooks.length === 0
                ? <EmptyState icon="📚" title={search ? 'No books match your search' : 'No books yet'} subtitle={search ? 'Try different keywords' : 'Add your first book above'} />
            ...
            {!loading && tab === 'members' && (
              filteredMembers.length === 0
                ? <EmptyState icon="👥" title={search ? 'No members match your search' : 'No members yet'} subtitle={search ? 'Try different keywords' : 'Add your first member above'} />
            ...
            {!loading && tab === 'borrow' && (
              filteredBorrows.length === 0
                ? <EmptyState icon="📖" title="No borrow records" subtitle={filter === 'active' ? 'No books currently out' : filter === 'returned' ? 'No returns yet' : 'Record a borrow to get started'} />
                : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        ...
                        <div key={rec.id} style={{ border: `0.5px solid ${overdue ? 'var(--color-red)' : 'var(--border-light)'}`, borderRadius: 'var(--radius-lg)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 150 }}>
                            ...
                            <p style={{ margin: '2px 0 0', fontSize: 13, color: 'var(--text-secondary)' }}>
                              {rec.member_name} · borrowed {fmtDate(rec.borrow_date)}
                            </p>
                          </div>
                          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            {active && (
                              <button onClick={() => handleReturn(rec.id)} disabled={returning === rec.id} style={{ fontSize: 13, opacity: returning === rec.id ? 0.5 : 1 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 100 }}
          ...
          <div style={{ background: 'var(--bg-primary)', borderRadius: 'var(--radius-lg)', border: '0.5px solid var(--border-light)', padding: '1.5rem', width: '100%', maxWidth: 440 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button onClick={closeModal} style={{ background: 'none', border: 'none', fontSize: 20, padding: 4, color: 'var(--text-secondary)' }}>✕</button>
            </div>
            ...
            {modal === 'add-member' && <>
              ...
              <FF label="Phone *">     <input value={form.phone || ''} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+1 555 000 0000" /></FF>
            </>}
            ...
            {modal === 'borrow' && <>
              ...
              {books.filter(b => b.available_copies > 0).length === 0 && (
                <p style={{ fontSize: 13, color: 'var(--color-amber)', margin: '0 0 12px' }}>⚠️ No books currently available to borrow.</p>
              )}
            </>}

            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 20 }}>
              <button onClick={closeModal}>Cancel</button>
              <button onClick={handleSubmit} disabled={submitting} style={{ opacity: submitting ? 0.6 : 1 }}>
